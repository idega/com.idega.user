...
public class BasicUserOverview extends Page implements IWBrowserView, StatefullPresentation {
	...
	private void addDeleteButton(EntityBrowser entityBrowser) {
		...
		if (this.hasDeletePermissionForRealGroup) {
			...
			confirmDeleting += " ?";
			SubmitButton deleteButton = new SubmitButton(this.iwrb.getLocalizedString("Delete selection", "Delete selection"), BasicUserOverview.DELETE_USERS_KEY, BasicUserOverview.DELETE_USERS_KEY);
	...
	protected EntityBrowser getEntityBrowser(Collection users, IWContext iwc) {
		...
		EntityToPresentationObjectConverter converterLink = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity, EntityPath path, EntityBrowser browser, IWContext iwc) {
				...
				else if (isUserSuperAdmin && BasicUserOverview.this.isCurrentUserSuperAdmin) {
		...
		String completeAddressKey = Address.class.getName() + ".STREET_NAME:" + Address.class.getName() + ".STREET_NUMBER:" + Address.class.getName() + ".P_O_BOX:" + PostalCode.class.getName() + ".POSTAL_CODE_ID|POSTAL_CODE:" + Address.class.getName() + ".CITY:" + Country.class.getName() + ".IC_COUNTRY_ID|COUNTRY_NAME";
