...
public class BasicUserOverview extends Page implements IWBrowserView,
		...
		try {
			if (this.selectedGroup != null) {
				if (this.aliasGroup != null) {
					users = getUserBusiness(iwc).getUsersInGroup(
							this.aliasGroup);
				} else {
					users = getUserBusiness(iwc).getUsersInGroup(
							this.selectedGroup);
	...
	protected Table getList(IWContext iwc) {
		...
		// get the data
		// this must be called here because classes that extend this class such
		// as the searchwindow class
		// do not necesserely need a selectedGroup to work
		Collection users = getEntries(iwc);

		if (this.selectedGroup != null) {
			// adds the name of the group (or alias group)
			// todo add the alias name and an explanation to what real group it
			// points to
			topTable.add(
					this.selectedGroup.getName() + Text.NON_BREAKING_SPACE, 1,
					1);
		}
		...
		if (users != null && !users.isEmpty()) {
			...
			LinkToUserStats linkToUserStats = (LinkToUserStats) ImplementorRepository
					.getInstance().newInstanceOrNull(LinkToUserStats.class,
							this.getClass());
			if (linkToUserStats != null) {
				/*
				 * linkToUserStats.setSelectedGroup(this.selectedGroup);
				 * linkToUserStats
				 * .setInvocationFileName("Invocation-UserStats.xml");
				 * linkToUserStats.setLayoutFileName("Layout-UserStats.xml");
				 * linkToUserStats
				 * .setLocalizableKeyName("userstatswindow.userstats");
				 */
			...
			/*
			 * LinkToUserStats linkToGroupStats =
			 * (LinkToUserStats)ImplementorRepository
			 * .getInstance().newInstanceOrNull(LinkToUserStats.class,
			 * this.getClass()); if (linkToGroupStats != null) {
			 * linkToGroupStats.setSelectedGroup(this.selectedGroup);
			 * linkToGroupStats
			 * .setInvocationFileName("Invocation-GroupStats.xml");
			 * linkToGroupStats.setLayoutFileName("Layout-GroupStats.xml");
			 * linkToGroupStats
			 * .setLocalizableKeyName("userstatswindow.groupstats"); Link link =
			 * linkToGroupStats.getLink();
			...
			// this can give inhereting classes a way to add something when
			...
			if (po != null) {
				// why a print button!??
				returnTable.add(
						new PrintButton(this.iwb.getImage("print.gif")), 1, 4);
	...
	protected void addEmailButton(EntityBrowser entityBrowser, IWContext iwc) {
		...
		if (this.hasEditPermissionForRealGroup && this.selectedGroup != null) {
			SubmitButton emailButton = new SubmitButton(
					this.iwrb.getLocalizedString("Email selection",
							"Email selection"),
			...
			iwc.setSessionAttribute(
					BasicUserOverviewEmailSenderWindow.PARAM_MAIL_SERVER,
					iwc.getApplicationSettings().getProperty(
							PROP_SYSTEM_SMTP_MAILSERVER));
			iwc.setSessionAttribute(
					BasicUserOverviewEmailSenderWindow.PARAM_FROM_ADDRESS,
					fromAddress);
			iwc.setSessionAttribute(
					BasicUserOverviewEmailSenderWindow.PARAM_SUBJECT,
					this.iwrb.getLocalizedString("to_members_in_group",
							"To members in group:")
							+ " "
							+ this.selectedGroup.getName());
		}
	}
	...
	private void addDeleteButton(EntityBrowser entityBrowser) {
		...
		if (this.hasDeletePermissionForRealGroup) {
			String confirmDeleting = this.iwrb.getLocalizedString(
					"buo_delete_selected_users", "Delete selected users");
			confirmDeleting += " ?";
			SubmitButton deleteButton = new SubmitButton(
					this.iwrb.getLocalizedString("Delete selection",
							"Delete selection"),
	...
	private void addMoveOrAddButton(EntityBrowser entityBrowser) {
		...
		if (this.selectedGroup == null) {
			...
			confirmMoving = this.iwrb.getLocalizedString(
					"buo_add_selected_users", "Add selected users");
			buttonMoving = this.iwrb.getLocalizedString("Add to", "Add to");
		} else {
			addMoveOrAddButton = this.hasEditPermissionForRealGroup;
			confirmMoving = this.iwrb.getLocalizedString(
					"buo_move_selected_users", "Move selected users");
			buttonMoving = this.iwrb.getLocalizedString("Move to", "Move to");
			confirmCopying = this.iwrb.getLocalizedString(
					"buo_copy_selected_users", "Copy selected users");
		...
		if (addMoveOrAddButton) {
			...
			if (this.selectedGroup != null) {
				SubmitButton copyToButton = new SubmitButton(buttonCopying,
						BasicUserOverview.COPY_USERS_KEY,
						BasicUserOverview.COPY_USERS_KEY);
				copyToButton.setSubmitConfirm(confirmCopying);
				styledCopyToButton = new StyledButton(copyToButton);
			}
			SubmitButton moveToButton = new SubmitButton(buttonMoving,
	...
	protected EntityBrowser getEntityBrowser(Collection users, IWContext iwc) {
		...
		EntityBrowser entityBrowser = EntityBrowser
				.getInstanceUsingEventSystemAndExternalForm();
		PresentationObject parentObject = this.getParentObject();
		entityBrowser
				.setArtificialCompoundId(parentObject.getCompoundId(), iwc);
		IWPresentationState presentationStateParent = ((StatefullPresentation) parentObject)
				.getPresentationState(iwc);
		IWPresentationState presentationStateChild = entityBrowser
				.getPresentationState(iwc);
		ChangeListener[] chListeners = presentationStateParent
				.getChangeListener();
		if (chListeners != null) {
		...
		entityBrowser
				.addActionListener((IWActionListener) presentationStateParent);
		...
		EntityToPresentationObjectConverter converterAddress = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getHeaderPresentationObject(
				...
				return browser.getDefaultConverter()
						.getHeaderPresentationObject(entityPath, browser, iwc);
			}
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				try {
					address = BasicUserOverview.getUserBusiness(iwc)
							.getUsersMainAddress(user);
				} catch (RemoteException ex) {
					System.err
							.println("[BasicUserOverview]: Address could not be retrieved.Message was : "
									+ ex.getMessage());
				...
				// default converter
				return (browser.getDefaultConverter().getPresentationObject(
						address, path, browser, iwc));
			}
		};
		...
		EntityToPresentationObjectConverter converterEmail = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				String emailString = "";
				try {
					email = BasicUserOverview.getUserBusiness(iwc).getUserMail(
							user);
					...
					System.err
							.println("[BasicUserOverview]: Email could not be retrieved.Message was :"
									+ ex.getMessage());
					ex.printStackTrace(System.err);
				}
				...
				// default converter
				// return
				// browser.getDefaultConverter().getPresentationObject((GenericEntity)
				// email, path, browser, iwc);
		...
		EntityToPresentationObjectConverter converterPhone = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				Phone[] phone = null;
				try {
					phone = BasicUserOverview.getUserBusiness(iwc)
							.getUserPhones(user);
				} catch (RemoteException ex) {
					System.err
							.println("[BasicUserOverview]: Phone could not be retrieved.Message was :"
									+ ex.getMessage());
					ex.printStackTrace(System.err);
				}
				// now the corresponding address was found, now just use the
				...
				for (i = 0; i < phone.length; i++) {
					table.add(getPresentationObjectForPhone(phone[i], path,
							browser, iwc));
				}
				return table;
			}

			private PresentationObject getPresentationObjectForPhone(
					Object genericEntity, EntityPath path,
					EntityBrowser browser, IWContext iwc) {
				StringBuffer displayValues = new StringBuffer();
				List list = path
						.getValues((EntityRepresentation) genericEntity);
				...
				while (valueIterator.hasNext()) {
					...
					if (object == null) {
						...
						if (PHONE_TYPE_PATH.equals(shortKey)) {
							object = getBundle(iwc).getResourceBundle(iwc)
									.getLocalizedString(phoneType, phoneType);
						}
					}
		...
		EntityToPresentationObjectConverter converterCompleteAddress = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(
				...
				displayValues.append(TextSoap.capitalize(getValue(0)))
						.append(' ').append(getValue(1));
				...
				if (displayValue.length() != 0) {
					displayValues.append(", ").append(getValue(3)).append(' ')
							.append(getValue(4));
				}
		...
		EntityToPresentationObjectConverter converterLink = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				PresentationObject text = browser.getDefaultConverter()
						.getPresentationObject(entity, path, browser, iwc);
				...
				boolean isUserSuperAdmin = user.getPrimaryKey().equals(
						getSuperAdmin(iwc).getPrimaryKey());

				if (!isUserSuperAdmin) {
					aLink.setWindowToOpen(UserPropertyWindow.class);
					aLink.addParameter(
							UserPropertyWindow.PARAMETERSTRING_USER_ID, user
									.getPrimaryKey().toString());

					if (BasicUserOverview.this.selectedGroup != null) {
						aLink.addParameter(
								UserPropertyWindow.PARAMETERSTRING_SELECTED_GROUP_ID,
								BasicUserOverview.this.selectedGroup
										.getPrimaryKey().toString());
					}

				} else if (isUserSuperAdmin
						&& BasicUserOverview.this.isCurrentUserSuperAdmin) {
		...
		EntityToPresentationObjectConverter converterToDeleteButton = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getHeaderPresentationObject(
				...
				checkAllCheckBox.setToCheckOnClick(
						BasicUserOverview.SELECTED_USERS_KEY, "this.checked");
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				if (!user.getPrimaryKey().equals(
						getSuperAdmin(iwc).getPrimaryKey())) {
					CheckBox checkBox = new CheckBox(
							BasicUserOverview.SELECTED_USERS_KEY,
							Integer.toString(user.getID()));
					
					Text row = new Text(Integer.toString(browser.getCurrentIndexOfEntities() + 1));
					
					layer.add(checkBox);
					layer.add(Text.NON_BREAKING_SPACE);
					layer.add(row);
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				try {
					if (BasicUserOverview.this.ps.getSelectedGroup() != null) {
						userStatusID = getUserStatusBusiness(iwc)
								.getUserGroupStatus(
										Integer.parseInt(user.getPrimaryKey()
												.toString()),
										Integer.parseInt(BasicUserOverview.this.ps
												.getSelectedGroup()
												.getPrimaryKey().toString()));
						if (userStatusID != -1) {
							status = getUserStatusBusiness(iwc)
									.getStatusHome()
									.findByPrimaryKey(new Integer(userStatusID));
						}
					}
				} catch (RemoteException ex) {
					System.err
							.println("[BasicUserOverview]: Status could not be retrieved.Message was :"
									+ ex.getMessage());
					ex.printStackTrace(System.err);
				...
				// Text text = new
				// Text(iwrb.getLocalizedString(browser.getDefaultConverter().getPresentationObject((GenericEntity)
				// status, path, browser,
				// iwc).toString(),browser.getDefaultConverter().getPresentationObject((GenericEntity)
				// status, path, browser, iwc).toString()));
				Text text = null;
				if (status != null) {
					text = new Text(iwrb.getLocalizedString(status
							.getStatusKey()));
				}
				return text;
			}
		...
		EntityToPresentationObjectConverter converterCustodianInfo = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				try {
					LinkToFamilyLogic linkToFamilyLogic = (LinkToFamilyLogic) ImplementorRepository
							.getInstance().newInstanceOrNull(
									LinkToFamilyLogic.class, this.getClass());
					if (linkToFamilyLogic != null) {
						custodians = linkToFamilyLogic.getCustodiansFor(user,
								iwc);
					}
					if (custodians != null) {
						...
						while (custIt.hasNext()) {
							...
							boolean isUserSuperAdmin = user.getPrimaryKey()
									.equals(getSuperAdmin(iwc).getPrimaryKey());

							if (!isUserSuperAdmin) {
								link.setWindowToOpen(UserPropertyWindow.class);
								link.addParameter(
										UserPropertyWindow.PARAMETERSTRING_USER_ID,
							...
							if (custEmail != null && !custEmail.isEmpty()) {
								...
								Email email = (Email) custEmail.iterator()
										.next();
								if (email.getEmailAddress() != null
									...
									emailString.append(email.getEmailAddress());
								}
								
								if (!"".equals(emailString.toString())) {
									linkContainer.add(" (");
									linkContainer.add(emailString.toString());
									linkContainer.add(")");
								}
							}
							if (custIt.hasNext()) {
		...
		EntityToPresentationObjectConverter converterUserInfoColumn1 = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity,
				...
				try {
					if (BasicUserOverview.this.ps.getSelectedGroup() != null) {
						infoColumns = getUserInfoColumnsBusiness(iwc)
								.getUserInfo(
										Integer.parseInt(user.getPrimaryKey()
												.toString()),
										Integer.parseInt(BasicUserOverview.this.ps
		...
		EntityToPresentationObjectConverter converterUserGroupRegistration = new EntityToPresentationObjectConverter() {
			...
			public PresentationObject getPresentationObject(Object entity,
					EntityPath path, EntityBrowser browser, IWContext iwc) {
				// entity is a user, try to get the corresponding registration date
				User user = (User) entity;
				IWTimestamp registerDate = null;
				try {
					if (BasicUserOverview.this.ps.getSelectedGroup() != null) {
						GroupRelationHome home = getGroupBusiness(iwc)
								.getGroupRelationHome();
						Collection registrations = home
								.findGroupsRelationshipsContainingGroupsAndStatus(
										BasicUserOverview.this.ps
												.getSelectedGroup(), user, 
										"ST_ACTIVE");

						if (registrations != null && !registrations.isEmpty()) {
							Iterator it = registrations.iterator();
							while (it.hasNext()) {
								GroupRelation relation = (GroupRelation) it
										.next();
								if (registerDate == null) {
									registerDate = new IWTimestamp(
											relation.getInitiationDate());
								} else {
									IWTimestamp tmp = new IWTimestamp(
											relation.getInitiationDate());
									if (tmp.isLaterThan(registerDate)) {
										registerDate = new IWTimestamp(
												relation.getInitiationDate());
									}
								}
				...
				if (registerDate == null) {
					...
					text = new Text(registerDate.getDateString("dd.MM.yyyy"));
				}
		...
		// set default columns
		String nameKey = User.class.getName() + ".FIRST_NAME:"
				+ User.class.getName() + ".MIDDLE_NAME:" + User.class.getName()
				+ ".LAST_NAME";
		String completeAddressKey = Address.class.getName() + ".STREET_NAME:"
				+ Address.class.getName() + ".STREET_NUMBER:"
				+ Address.class.getName() + ".P_O_BOX:"
				+ PostalCode.class.getName() + ".POSTAL_CODE_ID|POSTAL_CODE:"
				+ Address.class.getName() + ".CITY:" + Country.class.getName()
				+ ".IC_COUNTRY_ID|COUNTRY_NAME";
		String emailKey = Email.class.getName() + ".ADDRESS";
		String phoneKey = PHONE_TYPE_PATH + ":" + Phone.class.getName()
				+ ".PHONE_NUMBER";
		...
		String userGroupRegistrationKey = GroupRelation.class.getName() + ".INITIATION_DATE";
		...
		// set color of rows
		entityBrowser.setColorForEvenRows(IWColor.getHexColorString(246, 246,
				247));
		...
		entityBrowser.setDefaultColumn(1, displayNameKey);
		...
		entityBrowser.setEntityToPresentationConverter("Delete",
				converterToDeleteButton);
		entityBrowser.setEntityToPresentationConverter(nameKey,
				converterLink);
		entityBrowser.setEntityToPresentationConverter(displayNameKey,
				converterLink);
		entityBrowser.setEntityToPresentationConverter(completeAddressKey,
				converterCompleteAddress);
		entityBrowser.setEntityToPresentationConverter(dateOfBirthKey,
				new DateConverter());
		entityBrowser.setEntityToPresentationConverter(custodianKey,
				converterCustodianInfo);

		entityBrowser.setEntityToPresentationConverter(userInfo1Key,
				converterUserInfoColumn1);
		entityBrowser.setEntityToPresentationConverter(userInfo2Key,
				converterUserInfoColumn2);
		entityBrowser.setEntityToPresentationConverter(userInfo3Key,
				converterUserInfoColumn3);
		entityBrowser.setEntityToPresentationConverter(userGroupRegistrationKey,
				converterUserGroupRegistration);
		// set converter for all columns of this class
		entityBrowser.setEntityToPresentationConverter(Address.class.getName(),
				converterAddress);
		entityBrowser.setEntityToPresentationConverter(Email.class.getName(),
				converterEmail);
		entityBrowser.setEntityToPresentationConverter(Phone.class.getName(),
				converterPhone);
		entityBrowser.setEntityToPresentationConverter(Status.class.getName(),
				converterUserStatus);
		...
		entityBrowser.addEntity(GroupRelation.class.getName());
		...
		entityBrowser.setOptionColumn(3, nameKey);
		...
		entityBrowser.setOptionColumn(9, userGroupRegistrationKey);
		String displayDescription = iwc.getApplicationSettings().getProperty(
				"USER_APP_DISPLAY_DESCRIPTION", "true");
		if (IWContext.getInstance().isSuperAdmin()
				|| displayDescription.equalsIgnoreCase("true")) {
			entityBrowser.setOptionColumn(10, descriptionKey);
		}
		// change display
	...
	public void main(IWContext iwc) throws Exception {
		...
		if (getPresentationStateOfBasicUserOverview(iwc)
				.getResultOfMovingUsers() != null) {
			...
			if (this.hasViewPermissionForRealGroup
					|| this.hasPermitPermissionForRealGroup) {
				this.add(getList(iwc));
			} else {
				add(this.iwrb.getLocalizedString("no.view.permission",
						"You are not allowed to view the data for this group."));
			}
		} else {
		...
		String openSendMailWindow = (String) iwc
				.getSessionAttribute(OPEN_SEND_MAIL_WINDOW);
		if (openSendMailWindow != null
				&& openSendMailWindow.equalsIgnoreCase("true")) {
			Link l = new Link();
			l.setWindowToOpen(BasicUserOverviewEmailSenderWindow.class);
			parentPage.setOnLoad(l.getWindowToOpenCallingScript(iwc));
			// parentPage.getAssociatedScript().addFunction("",
			// l.getWindowToOpenCallingScript(iwc));
		}
	...
	/**
	 * sets the global access permission variables in this class for the current
	 * user
	 * 
	 * @param iwc
	 * @throws Exception
	...
	private void setAccessPermissions(IWContext iwc) throws Exception {
		...
		if (!this.isCurrentUserSuperAdmin) {
			if (this.aliasGroup != null) {// thats the real group
				this.hasOwnerPermissionForRealGroup = this.accessController
						.isOwner(this.aliasGroup, iwc);
				if (!this.hasOwnerPermissionForRealGroup) {
					this.hasViewPermissionForRealGroup = this.accessController
							.hasViewPermissionFor(this.aliasGroup, iwc);
					this.hasEditPermissionForRealGroup = this.accessController
							.hasEditPermissionFor(this.aliasGroup, iwc);
					this.hasDeletePermissionForRealGroup = this.accessController
							.hasDeletePermissionFor(this.aliasGroup, iwc);
					this.hasPermitPermissionForRealGroup = this.accessController
							.hasPermitPermissionFor(this.aliasGroup, iwc);
				} else {
			...
			} else if (this.selectedGroup != null) {// the third case:
													// selectedGroup == null
													// happens when doing a
													// search for example
				this.hasOwnerPermissionForRealGroup = this.accessController
						.isOwner(this.selectedGroup, iwc);
				if (!this.hasOwnerPermissionForRealGroup) {
					this.hasViewPermissionForRealGroup = this.accessController
							.hasViewPermissionFor(this.selectedGroup, iwc);
					this.hasEditPermissionForRealGroup = this.accessController
							.hasEditPermissionFor(this.selectedGroup, iwc);
					this.hasDeletePermissionForRealGroup = this.accessController
							.hasDeletePermissionFor(this.selectedGroup, iwc);
					this.hasPermitPermissionForRealGroup = this.accessController
							.hasPermitPermissionFor(this.selectedGroup, iwc);
				} else {
					// the user is the owner so he can do anything
					this.hasViewPermissionForRealGroup = true;
					this.hasEditPermissionForRealGroup = true;
	...
	com.idega.core.user.data.User getSuperAdmin(IWContext iwc) {
		if (this.administratorUser == null) {
			try {
				this.administratorUser = iwc.getAccessController()
						.getAdministratorUser();
			} catch (Exception ex) {
				System.err
						.println("[BasicUserOverview] access controller failed "
								+ ex.getMessage());
				ex.printStackTrace(System.err);
			}
		}
	...
	private void checkForAlias() {
		if (this.selectedGroup != null) {
			...
			// TODO PALLI: Have to fix this once and for all. It looks like the
			// class is cloned and not instanciated, so this variable doesn't
			// become null.
			else {
	...
	public static UserBusiness getUserBusiness(IWApplicationContext iwc) {
		...
		if (business == null) {
			try {
				business = (UserBusiness) com.idega.business.IBOLookup
	...
	public static GroupBusiness getGroupBusiness(IWApplicationContext iwc) {
		...
		if (business == null) {
			try {
				business = (GroupBusiness) com.idega.business.IBOLookup
	...
	public UserStatusBusiness getUserStatusBusiness(IWApplicationContext iwc) {
		...
		if (business == null) {
			try {
				business = (UserStatusBusiness) com.idega.business.IBOLookup
						.getServiceInstance(iwc, UserStatusBusiness.class);
			} catch (java.rmi.RemoteException rme) {
				throw new RuntimeException(rme.getMessage());
			}
		}
	...
	public UserInfoColumnsBusiness getUserInfoColumnsBusiness(
			IWApplicationContext iwc) {
		UserInfoColumnsBusiness business = null;
		if (business == null) {
			try {
				business = (UserInfoColumnsBusiness) com.idega.business.IBOLookup
						.getServiceInstance(iwc, UserInfoColumnsBusiness.class);
			} catch (java.rmi.RemoteException rme) {
				throw new RuntimeException(rme.getMessage());
			}
		}
	...
	public static List removeUsers(Collection userIds, Group parentGroup,
		...
		while (iterator.hasNext()) {
			...
			if ((userId = (String) iterator.next()) != null) {
				try {
					User currentUser = iwc.getCurrentUser();
					userBusiness.removeUserFromGroup(Integer.parseInt(userId),
							parentGroup, currentUser);
				} catch (RemoveException e) {
					System.err
							.println("[BasicUserOverview] "
									+ IWTimestamp.RightNow() + " user with id "
									+ userId + " could not be removed"
									+ e.getMessage());
	...
	public static synchronized Map moveContentOfGroups(Collection groups,
			Collection groupTypesToMoveAmong, IWContext iwc)
	...
	public IWPresentationState getPresentationState(IWUserContext iwuc) {
		if (this._presentationState == null) {
			try {
				...
				this._presentationState = (BasicUserOverviewPS) stateMachine
						.getStateFor(getCompoundId(),
								this.getPresentationStateClass());
	...
	private BasicUserOverviewPS getPresentationStateOfBasicUserOverview(
			IWUserContext iwuc) {
		try {
			IWStateMachine stateMachine = (IWStateMachine) IBOLookup
					.getSessionInstance(iwuc, IWStateMachine.class);
			String code = IWMainApplication
					.getEncryptedClassName(BasicUserOverview.class);
			code = ":" + code;
			return (BasicUserOverviewPS) stateMachine.getStateFor(code,
					BasicUserOverviewPS.class);
		} catch (RemoteException ex) {
	...
	private Table getResultList(IWContext iwc) {
		...
		String movedUsersNumberMessage = getLocalizedString(
				"number_of_sucessfully_moved_users",
				"Number of successfully moved users", iwc);
		String notMovedUsersNumberMessage = getLocalizedString(
				"number_of_not_moved_users", "Number of not moved users", iwc);
		String notMovedUsersMessage = getLocalizedString(
				"the_following_users_were_not moved",
				"Following users were not moved", iwc);
		String success = getLocalizedString(
				"all_users_were_moved_to the_specified_group",
				"All users were successfully moved.", iwc);
		...
		while (iterator.hasNext()) {
			...
			if ((new Integer(-1)).equals(groupId)) {
				...
				try {
					groupName = groupBusiness
							.getNameOfGroupWithParentName(group);
				} catch (RemoteException e) {
		...
		if (targetGroupId > 0) {
			...
			try {
				String targetName = biz
						.getNameOfGroupWithParentName(targetGroup);
		...
		Text movedUsersNumberMessageText = new Text(
		...
		Text notMovedUsersNumberMessageText = new Text(
				notMovedUsersNumberMessage);
		...
		if (notMovedUsers > 0) {
			EntityBrowser browser = getEntityBrowserForResult(
					notMovedUsersColl, completeResultOfMoving, state, iwc);
			// put print button to bottom
			browser.addPresentationObjectToBottom(new PrintButton(this.iwb
					.getImage("print.gif")));
	...
	private EntityBrowser getEntityBrowserForResult(Collection users,
			Map messageMap, BasicUserOverviewPS state, IWContext iwc) {
		// define entity browser
		EntityBrowser entityBrowser = EntityBrowser
				.getInstanceUsingEventSystem();
		PresentationObject parentObject = this.getParentObject();
		entityBrowser
				.setArtificialCompoundId(parentObject.getCompoundId(), iwc);
		IWPresentationState presentationStateParent = ((StatefullPresentation) parentObject)
		...
		String completeAddressKey = Address.class.getName() + ".STREET_NAME:"
				...
				+ ".IC_COUNTRY_ID|COUNTRY_NAME";
		String emailKey = Email.class.getName() + ".ADDRESS";
		String phoneKey = PhoneType.class.getName()
				+ ".IC_PHONE_TYPE_ID|TYPE_DISPLAY_NAME:"
				+ Phone.class.getName() + ".PHONE_NUMBER";
		String pinKey = User.class.getName() + ".PERSONAL_ID";

		Iterator iterator = messageMap.keySet().iterator();
		String identifier = (iterator.hasNext()) ? iterator.next().toString()
				: "move";
		...
		// set special converters
		entityBrowser.setEntityToPresentationConverter(errorMessageKey,
	...
	protected String getEntityBrowserIdentifier(BasicUserOverviewPS state) {
		...
		String identifier = (this.selectedGroup == null) ? ""
				: this.selectedGroup.getPrimaryKey().toString();
		identifier += "_";
		identifier += (state.getSelectedDomain() != null) ? state
				.getSelectedDomain().getPrimaryKey().toString() : "";
