...
public class CreateUserGroup extends Window {
	...
	public void lineUpElements(IWContext iwc) throws Exception {
		...
		if (size > 1) {
			...
			for (int i = 0; i < this.groupType.size(); i++) {
				...
				// String value =
				// ((Group)com.idega.user.data.GroupBMPBean.getStaticInstance((Class)groupType.get(i))).getGroupTypeValue();
	...
	// try{
	// business =
	// (UserGroupBusiness)com.idega.business.IBOLookup.getServiceInstance(iwc,UserGroupBusiness.class);
	// }
	// catch(java.rmi.RemoteException rme){
	// throw new RuntimeException(rme.getMessage());
	// }
