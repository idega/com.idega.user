...
public class GroupInfoViewer extends Block {
	...
	private void setPropertiesBean(String instanceId) {
		...
		properties.setGroupsIds(groupsIds);
		...
		WFUtil.invoke(UserConstants.GROUPS_MANAGER_BEAN_ID, "addProperties", parameters, classes);
	}
	...
	private void addJavaScript(IWContext iwc, String instanceId) {
		...
		action.append(instanceId).append("', '").append(GROUP_INFO_CONTAINER_ID).append("', 'Loading...").append("')});");
		parent.addJavaScriptAfterJavaScriptURLs("get_groups", action.toString());
	}
	
	public void setGroups(String server, String user, String password, List<String> groupsIds) {
		this.server = server;
		this.user = user;
